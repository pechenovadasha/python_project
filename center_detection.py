...
class PupilTracker:
    ...
    def process(self, diff_orig, bright_img, dir_name):
        ...
        roi_ready = (
            self.use_roi and
            self.roi_coords and
            self.roi_coords[0] is not None and
            self.roi_coords[1] is not None
        )

        if self.use_roi and not roi_ready:
            print("ROI requested but coords are missing, resetting ROI usage")
            self.use_roi = False
...
def process_frame(diff, dark_img, bright_img, dir_name):
    ...
    return tracker.process(diff, bright_img, dir_name)
